...
//    }
    static private boolean is_windows_drive_prefix(String filename) {
        ...
        return (c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z') && filename.charAt(1) == ':';
    }
...
//    }
    ...
    static private int find_image_format(String filename, BlockDriverRef pdrv)
    {
        ...
        ret = bdrv_pread(bs.value, buf, buf.length);
        bdrv_delete(bs.value);
        if (ret < 0) {
            pdrv.value = null;
            return ret;
    ...
    static private int bdrv_open_common(BlockDriverState bs, String filename, int flags, BlockDriver drv) {
        ...
        boolean use_bdrv_whitelist = false;
...
//        const char *action_str;
    ...
    static void bdrv_dev_change_media_cb(BlockDriverState bs) {
        if (bs.dev_ops!=null && bs.dev_ops.has_change_media_cb()) {
            ...
            bs.dev_ops.change_media_cb(bs.dev_opaque, true);
            if (tray_was_closed) {
                /* tray open */
...
//    }
    ...
    static int bdrv_read_unthrottled(BlockDriverState bs, byte[] buf)
    {
        boolean enabled;
        int ret;
...
//
    ...
    */
    ...
    static private int bdrv_pread(BlockDriverState bs, byte[] buf, int count1) {
        ...
        len = ((BDRV_SECTOR_SIZE) & (BDRV_SECTOR_SIZE - 1));
        if (len > count)
            len = count;
        sector_num = 0;
        int bufOffset = 0;
