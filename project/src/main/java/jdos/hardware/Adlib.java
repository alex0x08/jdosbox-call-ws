...
//    typedef /*Bit8u*/short RegisterCache[512];
    ...
    static private class Module extends Module_base {
        public Module(Section configuration) {
            super(configuration);
            IoHandler.IO_ReadHandleObject[] readHandler = new IoHandler.IO_ReadHandleObject[3];
            for (int i = 0; i< readHandler.length; i++)
                readHandler[i] = new IoHandler.IO_ReadHandleObject();
            IoHandler.IO_WriteHandleObject[] writeHandler = new IoHandler.IO_WriteHandleObject[3];
...
//            capture = null;
            ...
            /*Bitu*/int base = section.Get_hex("sbbase").toInt();
            /*Bitu*/int rate = section.Get_int("oplrate");
            //Make sure we can't select lower than 8000 to prevent fixed point issues
            if ( rate < 8000 )
                rate = 8000;
            String oplemu = section.Get_string( "oplemu" );
            Mixer.MixerObject mixerObject = new Mixer.MixerObject();
...
//        for ( int i = 0; i < 18; i++ ) {
//            /*Bit8u*/short* set = module.cache + ( i / 9 ) * 256;
//            /*Bitu*/int offset = ((i % 9) / 3) * 8 + (i % 3);
//            /*Bit8u*/short* base = set + offset;
...
//
//
//    static void OPL_SaveRawEvent(boolean pressed) {
//        if (!pressed)
//            return;
////	SaveRad();return;
