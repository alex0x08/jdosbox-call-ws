...
public class RasterizerCompiler extends RasterizerCompilerCommon {
    ...
    private static final Vector<SaveInfo> savedClasses = new Vector<>();
    ...
    static private void CLAMPED_Z(StringBuilder method, int fbzcp, String result) {
    ...
    static private void CLAMPED_W(StringBuilder method, int fbzcp, String result) {
    ...
    static private void compile(StringBuilder method, int tmuCount, int colorPath, int alphaMode, int fogMode, int fbzMode, int textureMode0, int textureMode1) {
        ...
        if (needDepthVal) {
            if (!VoodooCommon.FBZMODE_WBUFFER_SELECT(fbzMode)) {
                method.append(  "int ");CLAMPED_Z(method, colorPath, "depthval");
        ...
        if (uses_c_local) {
                ...
                switch (VoodooCommon.FBZCP_CCA_LOCALSELECT(colorPath))
            {
                ...
                case 2:     /* clamped iterated Z[27:20] */
                {
                    ...
                    CLAMPED_Z(method, colorPath, "temp");
                ...
                {
                    method.append("int temp = ");
                    CLAMPED_W(method, colorPath, "temp");
                    method.append("c_local = (c_local & 0x00FFFFFF) | (temp << 24);\n");
                    break;
                }
            }
            ...
            /* non-constant fog comes from several sources */
                ...
                switch (VoodooCommon.FOGMODE_FOG_ZALPHA(fogMode)) {
                    ...
                    case 1:     /* iterated A */
                        ...
                        break;
                    case 2:     /* iterated Z */
                        CLAMPED_Z(method, colorPath, "fogblend");
                        method.append("fogblend >>= 8;\n");
                        break;
                    case 3:     /* iterated W - Voodoo 2 only */
                        CLAMPED_W(method, colorPath, "fogblend");
                        break;
                }
    ...
    static private raster_info compileMethod(StringBuilder method, raster_info info) {
       ...
       try {
           ...
           codeBlock.setSuperclass(pool.getCtClass( raster_info.class.getName()));
           codeBlock.addInterface(pool.getCtClass(jdos.hardware.mame.poly_draw_scanline_func.class.getName()));
           method.append("}");
          ...
          "this.callback = this;}";
           ...
           raster_info result = (raster_info) clazz.getDeclaredConstructor().newInstance();
           if (saveClasses) {
