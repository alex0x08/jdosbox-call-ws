...
public class Mixer extends Program {
    ...
    * and removes itself when destroyed. */
    ...
    static private void MIXER_MixData(/*Bitu*/int needed) {
        ...
        if ((Hardware.CaptureState & (Hardware.CAPTURE_WAVE|Hardware.CAPTURE_VIDEO))!=0) {
            /*Bit16s*/short[] convert=new short[1024*2];
            /*Bitu*/int added=needed-mixer.done;
            if (added>1024)
                added=1024;
            /*Bitu*/int readpos=(mixer.pos+mixer.done)&MIXER_BUFMASK;
            for (/*Bitu*/int i=0;i<added;i++) {
                /*Bits*/int sample=mixer.work[readpos][0] >> MIXER_VOLSHIFT;
    ...
    public void Run() {
        ...
        if ((temp_line=cmd.FindString("MASTER",false))!=null) {
            MakeVolume(temp_line,mixer.mastervol[0],mixer.mastervol[1]);
        }
        MixerChannel chan=mixer.channels;
        while (chan!=null) {
            if ((temp_line=cmd.FindString(chan.name,false))!=null) {
                MakeVolume(temp_line,chan.volmain[0],chan.volmain[1]);
            }
            chan.UpdateVolume();
            chan=chan.next;
        }
    ...
    private static final PROGRAMS_Main MIXER_ProgramStart = Mixer::new;
