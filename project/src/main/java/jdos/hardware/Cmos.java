...
public class Cmos extends Module_base {
    ...
    static private final IoHandler.IO_WriteHandler cmos_writereg = (port, val, iolen) -> {
        ...
        default:
            cmos.regs[cmos.reg]=(byte)(val & 0x7f);
            Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_ERROR,"CMOS:WRite to unhandled register "+Integer.toString(cmos.reg,16));
        }
    };
    ...
    static private final IoHandler.IO_ReadHandler cmos_readreg = (port, iolen) -> {
        if (cmos.reg>0x3f) {
            Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_ERROR,"CMOS:Read from illegal register "+Integer.toString(cmos.reg,16));
            return 0xff;
        }
