...
public class IPX extends Module_base {
    ...
    private static class ECBClass {
        ...
        public void close() {
            if (Config.IPX_DEBUGMSG) {
                ECBAmount--;
                Log.log_msg(StringHelper.sprintf("ECB: SN%7d destroyed. Remaining ECBs: %3d", new Object[] {SerialNumber, ECBAmount}));
            }
    ...
    private static void handleIpxRequest() {
        ...
        switch (CPU_Regs.reg_ebx.word()) {
            case 0x0000:	// Open socket
                OpenSocket();
                Log.log_msg(StringHelper.sprintf("IPX: Open socket %4x", new Object[]{swapByte(CPU_Regs.reg_edx.word())}));
                break;
            case 0x0001:	// Close socket
                Log.log_msg(StringHelper.sprintf("IPX: Close socket %4x", new Object[]{swapByte(CPU_Regs.reg_edx.word())}));
                CloseSocket();
                break;
            ...
            case 0x0009:		// Get internetwork address
            {
                Log.log_msg(StringHelper.sprintf("IPX: Get internetwork address %2x:%2x:%2x:%2x:%2x:%2x", new Object[] {
                        localIpxAddr.netnode[5] & 0xFF, localIpxAddr.netnode[4] & 0xFF,
            ...
            default:
                Log.log_msg(StringHelper.sprintf("Unhandled IPX function: %4x", new Object[] {CPU_Regs.reg_ebx.word()}));
                break;
        }
    }
    ...
    static private void DisconnectFromServer(boolean unexpected) {
        ...
        try {receiverThread.join();} catch (Exception ignored){}
    }
    ...
    private static void sendPacket(ECBClass sendecb) {
        ...
        for(/*Bitu*/int m=0;m<4;m++) {
            if (localIpxAddr.netnum[m] != outbuffer[m + 0x6]) {
                isloopback = false;
                break;
            }
        }
    ...
    private static boolean ConnectToServer(String strAddr) {
        try {
            ...
            try {
                ...
                try {ipxClientSocket.close();} catch (Exception ignored) {}
                return false;
            }
            ...
            Log.log_msg(StringHelper.sprintf("IPX: Connected to server.  IPX address is %d:%d:%d:%d:%d:%d", new Object[]{localIpxAddr.netnode[0] & 0xFF, localIpxAddr.netnode[1] & 0xFF, localIpxAddr.netnode[2] & 0xFF, localIpxAddr.netnode[3] & 0xFF, localIpxAddr.netnode[4] & 0xFF, localIpxAddr.netnode[5] & 0xFF} ));
    ...
    private IPX(Section configuration) {
        ...
        //save registers
        Memory.phys_writeb(phyDospage,0xFA);    // CLI
