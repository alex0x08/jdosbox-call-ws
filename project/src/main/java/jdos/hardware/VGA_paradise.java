...
public class VGA_paradise {
    ...
    static private class SVGA_PVGA1A_DATA {
        ...
        /*Bitu*/final int[] clockFreq = new int[4];
        /*Bitu*/int biosMode;
    }

    static final SVGA_PVGA1A_DATA pvga1a = new SVGA_PVGA1A_DATA();
...
//       anything and actually works in WHATVGA just fine.
    ...
    /*Bitu*//*Bitu*//*Bitu*/
    static private final VGA.tWritePort write_p3cf_pvga1a = (reg, val, iolen) -> {
    ...
    /*Bitu*//*Bitu*//*Bitu*/
    static private final VGA.tReadPort read_p3cf_pvga1a = (reg, iolen) -> {
        if (pvga1a.locked() && reg >= 0x09 && reg <= 0x0e)
            return 0x0;
    ...
    /*Bitu*/
    private static final VGA.tFinishSetMode FinishSetMode_PVGA1A = (crtc_base, modeData) -> {
...
// Reset to single bank and set it to 0. May need to unlock first (DPaint locks on exit)
    ...
    private static final VGA.tDetermineMode DetermineMode_PVGA1A = () -> {
    ...
    /*Bitu*//*Bitu*/
    private static final VGA.tSetClock SetClock_PVGA1A = (which, target) -> {
        if (which < 4) {
    ...
    /*Bitu*/
    private static final VGA.tGetClock GetClock_PVGA1A = () -> pvga1a.clockFreq[(VGA.vga.misc_output >> 2) & 3];

    /*Bitu*/
    private static final VGA.tAcceptsMode AcceptsMode_PVGA1A = modeNo -> Int10_modes.VideoModeMemSize(modeNo) < VGA.vga.vmemsize;
