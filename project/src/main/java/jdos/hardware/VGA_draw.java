...
public class VGA_draw {
        ...
        public void call(/*Bitu*/int val) {
            ...
            if (Dosbox.IS_VGA_ARCH()) { // others need more investigation
                if (vbstart < vtotal) { // There will be no blanking at all otherwise
                    if (vbend > vtotal) {
                        ...
                        Log.log(LogTypes.LOG_VGA,LogSeverities.LOG_WARN,"Blanking wrap to line "+vblank_skip);
                    } else if (vbstart<=1) {
                        // blanking is used to cut lines at the start of the screen
                        vblank_skip = vbend;
                        Log.log(LogTypes.LOG_VGA,LogSeverities.LOG_WARN,"Upper "+vblank_skip+" lines of the screen blanked");
                    } else if (vbstart < vdend) {
                        if (vbend < vdend) {
                            // the game wants a black bar somewhere on the screen
                            Log.log(LogTypes.LOG_VGA,LogSeverities.LOG_WARN,"Unsupported blanking: line "+vbstart+"-"+vbend);
                        } else {
            ...
            */
            ...
            case VGA.M_HERC_TEXT:
                ...
                break;
            default:
                Log.log(LogTypes.LOG_VGA,LogSeverities.LOG_ERROR,"Unhandled VGA mode "+VGA.vga.mode+" while checking for resolution");
                break;
            }
            VGA_CheckScanLength();
    ...
    private static void VGA_KillDrawing() {
        ...
        VGA.vga.draw.lines_done = ~0;
        if (!VGA.vga.draw.vga_override) Render.RENDER_EndUpdate();
    }
