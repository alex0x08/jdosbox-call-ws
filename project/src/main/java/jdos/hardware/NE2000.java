...
public class NE2000 extends Module_base {
    ...
    public static final class bx_ne2k_c implements RxFrame {
        ...
        public /*Bit32u*/int read_cr() {
            ...
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] read CR returns 0x%08x", new Object[]{val}));
            return val;
        }

        public void write_cr(/*Bit32u*/int value) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] wrote 0x%02x to CR", new Object[]{value}));
...
//                #endif
        ...
        public /*Bit32u*/long chipmem_read(/*Bit32u*/int address, /*unsigned*/int io_len) {
            ...
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("out-of-bounds chipmem read, %04X", new Object[]{address}));
        ...
        public /*Bit32u*/long asic_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            ...
            switch (offset) {
                ...
                default:
                    if (BX_INFO)
                        Log.log_msg(StringHelper.sprintf("[NE2000] asic read invalid address %04x", new Object[]{offset}));
                    break;
            }
            return retval;
        }
        ...
        public void asic_write(/*Bit32u*/int offset, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] asic write addr=0x%02x, value=0x%04x", new Object[]{offset, value}));
            switch (offset) {
                ...
                default: // this is invalid, but happens under win95 device detection
                    if (BX_INFO)
                        Log.log_msg(StringHelper.sprintf("[NE2000] asic write invalid address %04x, ignoring", new Object[]{(long) offset}));
                    break;
            }
        }
        ...
        public /*Bit32u*/long page0_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] page 0 read from port %04x, len=%d", new Object[]{offset, io_len}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    Log.log_msg(StringHelper.sprintf("[NE2000] bad length! page 0 read from port %04x, len=%u", new Object[]{offset, io_len}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                case 0x3:  // BNRY
                    return (s.bound_ptr);
                case 0x4:  // TSR
                    return (((long) s.TSR.ow_coll << 7) |
                            ((long) s.TSR.cd_hbeat << 6) |
                            ((long) s.TSR.fifo_ur << 5) |
                            ((long) s.TSR.no_carrier << 4) |
                            ((long) s.TSR.aborted << 3) |
                            ((long) s.TSR.collided << 2) |
                            (s.TSR.tx_ok));
                case 0x5:  // NCR
                    return (s.num_coll);
                case 0x6:  // FIFO
                    ...
                    return (s.fifo);
                case 0x7:  // ISR
                    return (((long) s.ISR.reset << 7) |
                            ((long) s.ISR.rdma_done << 6) |
                            ((long) s.ISR.cnt_oflow << 5) |
                            ((long) s.ISR.overwrite << 4) |
                            ((long) s.ISR.tx_err << 3) |
                            ((long) s.ISR.rx_err << 2) |
                            ((long) s.ISR.pkt_tx << 1) |
                            (s.ISR.pkt_rx));
                ...
                case 0xb:  // reserved
                    ...
                    return (0xff);
                case 0xc:  // RSR
                    return (((long) s.RSR.deferred << 7) |
                            ((long) s.RSR.rx_disabled << 6) |
                            ((long) s.RSR.rx_mbit << 5) |
                            ((long) s.RSR.rx_missed << 4) |
                            ((long) s.RSR.fifo_or << 3) |
                            ((long) s.RSR.bad_falign << 2) |
                            ((long) s.RSR.bad_crc << 1) |
                            (s.RSR.rx_ok));
                case 0xd:  // CNTR0
                    return (s.tallycnt_0);
                case 0xe:  // CNTR1
                ...
                default:
                    if (BX_PANIC)
                        Log.log_msg(StringHelper.sprintf("[NE2000] page 0 read offset %04x out of range", new Object[]{offset}));
            }
            return (0);
        }
        ...
        public void page0_write(/*Bit32u*/int offset, /*Bit32u*/long val, /*unsigned*/int io_len) {
            int value = (int) val;
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] page 0 write to port %04x, len=%d", new Object[]{offset, io_len}));
        ...
        public /*Bit32u*/long page1_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] page 1 read from port %04x, len=%d", new Object[]{offset, io_len}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    Log.log_msg(StringHelper.sprintf("[NE2000] bad length! page 1 read from port %04x, len=%u", new Object[]{offset, io_len}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                default:
                    if (BX_PANIC)
                        Log.log_msg(StringHelper.sprintf("[NE2000] page 1 read offset %04x out of range", new Object[]{offset}));
            }
            return 0;
        }
        ...
        public void page1_write(/*Bit32u*/int offset, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] page 1 write to port %04x, len=%d", new Object[]{offset, io_len}));
        ...
        public /*Bit32u*/long page2_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] page 2 read from port %04x, len=%d", new Object[]{offset, io_len}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    Log.log_msg(StringHelper.sprintf("[NE2000] bad length! page 2 read from port %04x, len=%u", new Object[]{offset, io_len}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                case 0xb:
                    ...
                    return (0xff);
                case 0xc:  // RCR
                    return (((long) s.RCR.monitor << 5) |
                            ((long) s.RCR.promisc << 4) |
                            ((long) s.RCR.multicast << 3) |
                            ((long) s.RCR.broadcast << 2) |
                            ((long) s.RCR.runts_ok << 1) |
                            (s.RCR.errors_ok));
                case 0xd:  // TCR
                    return (((long) s.TCR.coll_prio << 4) |
                            ((long) s.TCR.ext_stoptx << 3) |
                            ((s.TCR.loop_cntl & 0x3) << 1) |
                            (s.TCR.crc_disable));
                case 0xe:  // DCR
                    return (((s.DCR.fifo_size & 0x3) << 5) |
                            ((long) s.DCR.auto_rx << 4) |
                            ((long) s.DCR.loop << 3) |
                            ((long) s.DCR.longaddr << 2) |
                            ((long) s.DCR.endian << 1) |
                            (s.DCR.wdsize));
                case 0xf:  // IMR
                    return (((long) s.IMR.rdma_inte << 6) |
                            ((long) s.IMR.cofl_inte << 5) |
                            ((long) s.IMR.overw_inte << 4) |
                            ((long) s.IMR.txerr_inte << 3) |
                            ((long) s.IMR.rxerr_inte << 2) |
                            ((long) s.IMR.tx_inte << 1) |
                            (s.IMR.rx_inte));
                default:
                    if (BX_PANIC)
                        Log.log_msg(StringHelper.sprintf("[NE2000] page 2 read offset %04x out of range", new Object[]{offset}));
            }
            return 0;
        }
        ...
        public /*Bit32u*/long read(/*Bit32u*/long address, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                Log.log_msg(StringHelper.sprintf("[NE2000] read addr %x, len %d", new Object[]{address, io_len}));
            /*Bit32u*/
            long retval = 0;
         ...
         */
        public void rx_frame(Ptr buf, /*unsigned*/int io_len) {
...
//                     pktbuf[6], pktbuf[7], pktbuf[8], pktbuf[9], pktbuf[10], pktbuf[11]);
            ...
            if ((nextpage > s.curr_page) || ((s.curr_page + pages) == s.page_stop)) {
                ...
                Ptr.memcpy(startptr, new Ptr(buf, endbytes - 4), io_len - endbytes + 4);
            }
            s.curr_page = (short) nextpage;

            s.RSR.rx_ok = 1;
            if ((buf.readb(0) & 0x80) != 0) {
                s.RSR.rx_mbit = 1;
            }
