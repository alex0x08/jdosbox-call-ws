...
public class Memory extends Module_base {
    ...
    */
    ...
    private static class MemoryBlock {
        ...
        final LinkBlock links = new LinkBlock();
        ...
        public final Vector<PCI_PageHandler> pci = new Vector<>();
        public final Vector<ROM> roms = new Vector<>();
        public static class A20 {
            boolean enabled;
            /*Bit8u*/short controlport;
        }
        final A20 a20 = new A20();
    }
    ...
    static private class IllegalPageHandler extends Paging.PageHandler {
        ...
        public /*Bitu*/int readb(/*PhysPt*/int addr) {
            if (Config.C_DEBUG)
                Log.log_msg(StringHelper.sprintf("Illegal read from %x, CS:IP %8x:%8x",new Object[] {addr, CPU_Regs.reg_csVal.dword, CPU_Regs.reg_eip}));
            else {
                if (r_lcount<1000) {
                    r_lcount++;
                    Log.log_msg(StringHelper.sprintf("Illegal read from %x, CS:IP %8x:%8x",new Object[] {addr, CPU_Regs.reg_csVal.dword, CPU_Regs.reg_eip}));
        ...
        public void writeb(/*PhysPt*/int addr,/*Bitu*/int val) {
            if (Config.C_DEBUG)
                Log.log_msg(StringHelper.sprintf("Illegal write to %x, CS:IP %8x:%8x",new Object[] {addr, CPU_Regs.reg_csVal.dword, CPU_Regs.reg_eip}));
            else {
    ...
    static private class ROMPageHandler extends RAMPageHandler {
        ...
        public void writeb(/*PhysPt*/int addr,/*Bitu*/int val){
            Log.log(LogTypes.LOG_CPU,LogSeverities.LOG_ERROR,"Write "+Integer.toString(val, 16)+" to rom at "+Integer.toString(addr,16));
        }
    ...
    static public /*Bitu*/int MEM_TotalPages() {
        return memory==null? 0 : memory.pages;
    }
    ...
    */
    ...
    public Memory(Section configuration) {
        ...
        IoHandler.IO_WriteHandleObject writeHandler = new IoHandler.IO_WriteHandleObject();
        writeHandler.Install(0x92,write_p92,IoHandler.IO_MB);
        IoHandler.IO_ReadHandleObject readHandler = new IoHandler.IO_ReadHandleObject();
    ...
    static Memory test;
    public static final Section.SectionFunction MEM_ShutDown = new Section.SectionFunction() {
        public void call(Section section) {
            RAM.free();
            test = null;
        }
    };
    public static final Section.SectionFunction MEM_Init = new Section.SectionFunction() {
        public void call(Section section) {
