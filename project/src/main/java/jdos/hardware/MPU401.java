...
public class MPU401 extends Module_base {
    ...
    private static final IoHandler.IO_WriteHandler MPU401_WriteCommand = new IoHandler.IO_WriteHandler() {
        public void call(/*Bitu*/int port, /*Bitu*/int val, /*Bitu*/int iolen) {
            ...
            if (val<=0x2f) {
                ...
                if ((val&0x20)!=0) Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,"MPU-401:Unhandled Recording Command "+Integer.toString(val,16));
                switch (val&0xc) {
                    case  0x4:	/* Stop */
    ...
    private static /*Bitu*/int length;
        ...
        public void call(/*Bitu*/int port, /*Bitu*/int val, /*Bitu*/int iolen) {
            ...
            switch (mpu.state.data_onoff) { /* Data */
                ...
                case    1: /* MIDI */
                    ...
                    int posd = mpu.playbuf[mpu.state.channel].vlength;
                    if (posd ==1) {
                        switch (val&0xf0) {
    ...
    public MPU401(Section configuration) {
        ...
        installed = true;
        IoHandler.IO_WriteHandleObject[] writeHandler = new IoHandler.IO_WriteHandleObject[2];
        for (int i = 0; i< writeHandler.length; i++) {
            writeHandler[i] = new IoHandler.IO_WriteHandleObject();
        }
        IoHandler.IO_ReadHandleObject[] readHandler = new IoHandler.IO_ReadHandleObject[2];
