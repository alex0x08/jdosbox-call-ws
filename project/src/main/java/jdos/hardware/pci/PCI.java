...
public class PCI extends Module_base {
    ...
    private static final /*Bit8u*/byte[][][] pci_cfg_data = new byte[PCI_MAX_PCIDEVICES][PCI_MAX_PCIFUNCTIONS][256];		// PCI configuration data
    private static final PCI_Device[] pci_devices =new PCI_Device[PCI_MAX_PCIDEVICES];		// registered PCI devices
    ...
    static private final IoHandler.IO_WriteHandler write_pci_addr = (port, val, iolen) -> pci_caddress=val;
    ...
    static private final IoHandler.IO_WriteHandler write_pci = (port, val, iolen) -> {
        ...
        if ((pci_caddress & 0x80ff0000) == 0x80000000) {
            ...
            switch (iolen) {
                case 1: write_pci_register(dev, regnum,(val&0xff)); break;
                case 2: write_pci_register(dev, regnum,(val&0xff));
                        write_pci_register(dev,regnum+1,((val>>8)&0xff)); break;
                case 4: write_pci_register(dev, regnum,(val&0xff));
    ...
    static private final IoHandler.IO_ReadHandler read_pci_addr = (port, iolen) -> pci_caddress;
    ...
    static private final IoHandler.IO_ReadHandler read_pci = (port, iolen) -> {
   ...
   // private static final PCI_Device[] rqueued_devices = new PCI_Device[max_rqueued_devices];

    static public PCI pci_interface=null;

	private boolean initialized;

	protected final IoHandler.IO_WriteHandleObject[] PCI_WriteHandler = new IoHandler.IO_WriteHandleObject[5];
	protected final IoHandler.IO_ReadHandleObject[] PCI_ReadHandler = new IoHandler.IO_ReadHandleObject[5];

	protected final Callback callback_pci = new Callback();
    ...
    public void RegisterPCIDevice(PCI_Device device, int slot) {
		if (device==null) return;

		if (slot>=0) {
			// specific slot specified, basic check for validity
			if (slot>=PCI_MAX_PCIDEVICES) return;
		} else {
			// auto-add to new slot, check if one is still free
			if (pci_devices_installed>=PCI_MAX_PCIDEVICES) return;
		}
    ...
    public static final Section.SectionFunction PCI_ShutDown = section -> {
        pci_interface.destroy();
        pci_interface=null;
    };

    public static final Section.SectionFunction PCI_Init = sec -> {
