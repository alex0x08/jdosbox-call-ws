...
public class Callback {
   ...
   static public final int CB_INT21=20;
    ...
    static public final Handler[] CallBack_Handlers = new Handler[CB_MAX];
    ...
    static private final Handler default_handler = new Handler() {
        public /*Bitu*/int call() {
            Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,"Illegal Unhandled Interrupt Called "+Integer.toString(CPU.lastint,16));
            return CBRET_NONE;
        }
        public String getName() {
    ...
    public static void CALLBACK_Setup(/*Bitu*/int callback, Handler handler,/*Bitu*/int type, String descr) {
        if (callback>=CB_MAX) return;
        CALLBACK_SetupExtra(callback,type, CALLBACK_PhysPointer(callback),(handler!=null));
        ...
        if(m_type == SETUP) {
            if(vectorhandler.installed){
                ...
                if(Memory.RealGetVec(vectorhandler.interrupt) == Get_RealPointer()) {
                    ...
                    Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_WARN,"Interrupt vector changed on "+Integer.toString(vectorhandler.interrupt, 16)+" "+CALLBACK_GetDescription(m_callback));
            }
    ...
    public static final Section.SectionFunction CALLBACK_Init = new Section.SectionFunction() {
        public void call(Section section) {
            ...
            Memory.phys_writeb(CALLBACK_PhysPointer(call_priv_io),0xec);	// in al, dx
