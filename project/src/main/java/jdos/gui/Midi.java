...
public class Midi extends Module_base {
    ...
    static public void MIDI_RawOutByte(/*Bit8u*/int data) {
        if (midi.sysex.start!=0) {
            ...
            if (passed_ticks < midi.sysex.delay) try {Thread.sleep(midi.sysex.delay - passed_ticks);} catch (InterruptedException ignored){}
        }
        /* Test for a realtime MIDI message */
        if (data>=0xf8) {
            try {msg.setMessage(data);} catch (Exception ignored) {}
    ...
    //				LOG(LOG_ALL,LOG_NORMAL)("Play sysex; address:%02X %02X %02X, length:%4d, delay:%3d", midi.sysex.buf[5], midi.sysex.buf[6], midi.sysex.buf[7], midi.sysex.used, midi.sysex.delay);
                    try {sysex_msg.setMessage(midi.sysex.buf, midi.sysex.used);} catch (Exception ignored) {}
...
//                }
        ...
        if (midi.cmd_len!=0) {
            ...
            if (midi.cmd_pos >= midi.cmd_len) {
                try {msg.setMessage(midi.cmd_buf[0], midi.cmd_buf[1], midi.cmd_buf[2]);} catch (Exception ignored) {}
                midi.handler.send(msg, -1);
