...
public class Dos_misc {
    ...
    static private final Callback.Handler INT2F_Handler = new Callback.Handler() {
        public String getName() {
            return "Dos_misc.INT2F_Handler";
        }
        public /*Bitu*/int call() {
            for (int i=0;i<Multiplex.size();i++) {
                Dos_system.MultiplexHandler m = Multiplex.elementAt(i);
                if (m.call())
                    return Callback.CBRET_NONE;
            }
            Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,"DOS:Multiplex Unhandled call "+Integer.toString(CPU_Regs.reg_eax.word(),16));
            return Callback.CBRET_NONE;
        }
    };

    static private final Callback.Handler INT2A_Handler = new Callback.Handler() {
        public String getName() {
    ...
    static final Dos_system.MultiplexHandler DOS_MultiplexFunctions = () -> {
        ...
        case 0x1216:	/* GET ADDRESS OF SYSTEM FILE TABLE ENTRY */
            ...
            Log.log(LogTypes.LOG_DOSMISC,LogSeverities.LOG_ERROR,"Some BAD filetable call used bx="+Integer.toString(CPU_Regs.reg_ebx.word(),16));
            Callback.CALLBACK_SCF(CPU_Regs.reg_ebx.word() > Dos_files.DOS_FILES);
            if (CPU_Regs.reg_ebx.word()<16) {
                ...
                if (dotpos>=0) {
                    ...
                    /*PhysPt*/
                    for (i=0; i<nlen; i++)
                        Memory.mem_writeb(sftptr+sftofs+0x20+i,filename.charAt(i));
                    /*PhysPt*/
                    for (i=nlen; i<8; i++)
                        Memory.mem_writeb(sftptr+sftofs+0x20+i,' ');

                    if (extlen>3) extlen=3;
                    /*PhysPt*/
                    for (i=0; i<extlen; i++)
                        Memory.mem_writeb(sftptr+sftofs+0x28+i,filename.charAt(dotpos+i));
                    /*PhysPt*/
                    for (i=extlen; i<3; i++)
                        Memory.mem_writeb(sftptr+sftofs+0x28+i,' ');
                } else {
                    int i;
                    int nlen=filename.length();
                    if (nlen>8) nlen=8;
        ...
        case 0x1607:
            ...
            else return CPU_Regs.reg_ebx.word() == 0x18;	// idle callout
    ...
    static public void DOS_SetupMisc() {
        ...
        /*Bitu*/
        int call_int2f = Callback.CALLBACK_Allocate();
        ...
        int call_int2a = Callback.CALLBACK_Allocate();
