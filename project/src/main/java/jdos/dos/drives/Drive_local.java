...
public class Drive_local extends Dos_Drive {
    static public class localFile extends DOS_File {
        ...
        public boolean Read(byte[] data,/*Bit16u*/IntRef size) {
            ...
            try {size.value=fhandle.read(data,0,size.value);} catch (Exception ignored) {}
        ...
        public boolean Write(byte[] data,/*Bit16u*/IntRef size) {
            ...
            if(size.value==0){
                try {fhandle.setLength(fhandle.getFilePointer());} catch (Exception ignored){}
            }
            else
            {
                try {fhandle.write(data,0,size.value);} catch (Exception ignored) {}
            }
            return true;
        }
        ...
        public boolean Close() {
            ...
            if (refCtr==1) {
                if(fhandle!=null) try {fhandle.close();} catch (Exception ignored){}
                fhandle = null;
                open = false;
            }
            return true;
    ...
    public DOS_File FileOpen(String name,/*Bit32u*/int flags) {
        int type;
        switch (flags&0xf) {
        case Dos_files.OPEN_READ:
            case Dos_files.OPEN_READ_NO_MOD:
                type=FileIOFactory.MODE_READ; break;
        case Dos_files.OPEN_WRITE:type=FileIOFactory.MODE_WRITE; break;
        case Dos_files.OPEN_READWRITE:type=FileIOFactory.MODE_READ|FileIOFactory.MODE_WRITE; break; //No modification of dates. LORD4.07 uses this
            default:
