...
public class DOS_Drive_Cache {
    final Comparator<CFileInfo> SortByDirName = (o1, o2) -> {
        if (o1.isDir!= o2.isDir) return o1.isDir?1:-1;
        return o1.shortname.compareTo(o2.shortname);
    };

    final Comparator<CFileInfo> SortByDirNameRev = (o1, o2) -> {
        if (o1.isDir!= o2.isDir) return o1.isDir?1:-1;
        return o2.shortname.compareTo(o1.shortname);
    };

    final Comparator<CFileInfo> SortByNameRev = (o1, o2) -> o2.shortname.compareTo(o1.shortname);
    ...
    void SetDirSort () {
        sortDirType = TDirSort.DIRALPHABETICAL;
    }
    ...
    void ReadDir(/*Bit16u*/int id, StringRef result) {
        // shouldnt happen...
        if (id>MAX_OPENDIRS) return;
        ...
        if (SetResult(dirSearch[id], result, dirSearch[id].nextEntry)) return;
        if (dirSearch[id]!=null) {
            dirSearch[id].id = MAX_OPENDIRS;
            dirSearch[id] = null;
        }
    ...
    public boolean FindFirst(String path, IntRef id) {
        ...
        for (/*Bitu*/int i=0; i<dirSearch[dirID.value].fileList.size(); i++) {
            CopyEntry(dirFindFirst[dirFindFirstID], dirSearch[dirID.value].fileList.elementAt(i));
        }
    ...
    public boolean FindNext(/*Bit16u*/int id, StringRef result) {
        ...
        if ((id>=MAX_OPENDIRS) || dirFindFirst[id]==null) {
            Log.log(LogTypes.LOG_DOSMISC,LogSeverities.LOG_ERROR,"DIRCACHE: FindFirst/Next failure : ID out of range: "+Integer.toString(id,16));
        return false;
        }
    ...
    void ClearFileInfo(CFileInfo dir) {
        for(/*Bit32u*/int i=0; i<dir.fileList.size(); i++) {
            CFileInfo info = dir.fileList.elementAt(i);
            if (info!=null)
                ClearFileInfo(info);
        }
    ...
    public void CacheOut(String path, boolean ignoreLastDir/* = false*/) {
        ...
        for(/*Bit32u*/int i=0; i<dir.fileList.size(); i++) {
            ...
            DeleteFileInfo(dir.fileList.elementAt(i));
    ...
    static private class CFileInfo {
        ...
        /*Bitu*/int	    shortNr;
        // contents
        final Vector<CFileInfo>	fileList = new Vector<>();
        final Vector<CFileInfo>	longNameList = new Vector<>();
    }

    private void RemoveTrailingDot(StringRef shortname) {
        ...
        if (len>0 && (shortname.value.charAt(len-1)=='.')) {
            if (len==1) return;
            if ((len==2) && (shortname.value.charAt(0)=='.')) return;
    ...
    private /*Bits*/int GetLongName(CFileInfo curDir, StringRef shortName) {
        ...
        while (low<=high) {
            mid = (low+high)/2;
            res = shortName.value.compareTo(curDir.fileList.elementAt(mid).shortname);
            if (res>0)	low  = mid+1; else
            if (res<0)	high = mid-1; else
            {	// Found
                shortName.value = curDir.fileList.elementAt(mid).orgname;
                return mid;
            }
        }
        // not available
    ...
    private void CreateShortName(CFileInfo curDir, CFileInfo info) {
        ...
        if (createShort) {
            ...
            if (curDir.longNameList.size()>0) {
                if (info.shortname.compareTo(curDir.longNameList.lastElement().shortname)>=0) {
                    ...
                    for (i=0;i<curDir.longNameList.size();i++) {
                        CFileInfo it = curDir.longNameList.elementAt(i);
    ...
    private /*Bitu*/int CreateShortNameID (CFileInfo curDir, String name) {
        ...
        while (low<=high) {
            mid = (low+high)/2;
            res = CompareShortname(name, curDir.longNameList.elementAt(mid).shortname);
            ...
            if (res<0)	high = mid-1;
                ...
                do {
                    foundNr = curDir.longNameList.elementAt(mid).shortNr;
                    mid++;
                } while(mid<curDir.longNameList.size() && (CompareShortname(name, curDir.longNameList.elementAt(mid).shortname)==0));
                break;
            }
        }
        return foundNr+1;
    ...
    private boolean SetResult(CFileInfo dir, StringRef result, /*Bitu*/int entryNr) {
        ...
        CFileInfo info = dir.fileList.elementAt(entryNr);
    ...
    private CFileInfo FindDirInfo(String path, StringRef expandedPath) {
        ...
        do {
            ...
            // Error check
            // Follow Directory
            if ((nextDir>=0) && curDir.fileList.elementAt(nextDir).isDir) {
                curDir = curDir.fileList.elementAt(nextDir);
    ...
    private void CreateEntry(CFileInfo dir, String name, boolean is_directory) {
        ...
        if (dir.fileList.size()>0) {
            if (!(info.shortname.compareTo(dir.fileList.lastElement().shortname)<0)) {
                // append at end of list
                ...
                for (it=0;it<dir.fileList.size(); ++it) {
                    if (info.shortname.compareTo(dir.fileList.elementAt(it).shortname)<0) {
                        found = true;
                        break;
                    }
                }
