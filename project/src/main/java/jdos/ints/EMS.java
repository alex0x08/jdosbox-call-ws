...
public class EMS extends Module_base {
 ...
 //   static final private int EMM_MOVE_OVLAPI = 0x97;
    ...
    private static /*Bit8u*/void EMM_AllocateSystemHandle(/*Bit16u*/) {
        /* Check for enough free pages */
        if ((Memory.MEM_FreeTotal()/ 4) < 8) { return;}
        ...
        /*MemHandle*/int mem = Memory.MEM_AllocatePages(8 *4,false);
        if (mem==0) Log.exit("EMS:System handle memory allocation failure");
        emm_handles[handle].pages = 8;
        emm_handles[handle].mem = mem;
    }
    ...
    private static /*Bit8u*/short EMM_PartialPageMapping() {
        ...
        case 0x02:	/* Get Partial Page Map Array Size */
            ...
            break;
        default:
            Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_ERROR,"EMS:Call "+Integer.toString(CPU_Regs.reg_eax.high(), 16)+" Subfunction "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
    ...
    private static final Callback.Handler INT67_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            case 0x5A:              /* Allocate standard/raw Pages */
                if (CPU_Regs.reg_eax.low()<=0x01) {
                    ...
                    Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_ERROR,"EMS:Call 5A subfct "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
                    CPU_Regs.reg_eax.high(EMM_INVALID_SUB);
                }
                break;
            case 0xDE:		/* VCPI Functions */
                if (!vcpi.enabled) {
                    Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_ERROR,"EMS:VCPI Call "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
        ...
        //				MEM_A20_Enable(true);
                        ...
                        break;
                    default:
                        Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_ERROR,"EMS:VCPI Call "+Integer.toString(CPU_Regs.reg_eax.word(), 16)+" not supported");
                        CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                        break;
                    }
                }
                break;
            default:
                Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_ERROR,"EMS:Call "+Integer.toString(CPU_Regs.reg_eax.high(), 16)+" not supported");
                CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                break;
            }
    ...
    private static final Callback.Handler VCPI_PM_Handler = new Callback.Handler() {
        ...
        //		MEM_A20_Enable(false);
                ...
                break;
            default:
                Log.log(LogTypes.LOG_MISC,LogSeverities.LOG_WARN,"Unhandled VCPI-function "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" in protected mode");
                break;
            }
            return Callback.CBRET_NONE;
        }
    ...
    EMS(Section configuration) {
        ...
        Callback call_vdma = new Callback();
        ...
        EMM_AllocateSystemHandle();	// allocate OS-dedicated handle (ems handle zero, 128kb)
        ...
        if (ems_type!=2) {
            ...
            Callback call_vcpi = new Callback();
            ...
            Callback call_v86mon = new Callback();
