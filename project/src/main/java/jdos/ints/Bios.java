...
//        } else {
    ...
    private static void Tandy_SetupTransfer(/*PhysPt*/int bufpt,boolean isplayback) {
        ...
        /*Bit8u*/short tandy_irq;
        if (tandy_sb.port!=0) tandy_irq = tandy_sb.irq;
        else tandy_irq = tandy_dac.irq;
    ...
    private static final Callback.Handler INT1A_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            case 0x80:	/* Pcjr Setup Sound Multiplexer */
                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,"INT1A:80:Setup tandy sound multiplexer to "+ CPU_Regs.reg_eax.low());
                break;
            case 0x81:	/* Tandy sound system check */
            case 0x82:	/* Tandy sound system start recording */
            case 0x83:	/* Tandy sound system start playback */
            ...
            case 0xb1:		/* PCI Bios Calls */
                Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_WARN,"INT1A:PCI bios call "+Integer.toString(CPU_Regs.reg_eax.low(),16));
                if (!Config.PCI_FUNCTIONALITY_ENABLED) {
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        default:
                            Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_ERROR,"INT1A:PCI BIOS: unknown function "+Integer.toString(CPU_Regs.reg_eax.word(), 16)+" ("+Integer.toString(CPU_Regs.reg_ebx.word(), 16)+" "+Integer.toString(CPU_Regs.reg_ecx.word(), 16)+" "+Integer.toString(CPU_Regs.reg_edx.word(), 16)+")");
                            Callback.CALLBACK_SCF(true);
                            break;
                    }
                }
                ...
                break;
            default:
                Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_ERROR,"INT1A:Undefined call "+Integer.toString(CPU_Regs.reg_eax.high(),16));
            }
            return Callback.CBRET_NONE;
        }
    };
...
//    #undef DOSBOX_CLOCKSYNC
            ...
            case 0x87:	/* Copy extended memory */
                ...
                case 0xE8:
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        break;
                        default:
                            Log.log(LogTypes.LOG_BIOS,LogSeverities.LOG_ERROR,"INT15:Unknown call "+Integer.toString(CPU_Regs.reg_eax.word(),16));
    ...
    public Bios(Section configuration) {
        super(configuration);
        Callback[] callback = new Callback[11];
