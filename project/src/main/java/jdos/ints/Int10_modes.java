...
public class Int10_modes {
    ...
    private static void FinishSetMode(boolean clearmem) {
        ...
        if (CurMode.mode < 128) Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MODE, CurMode.mode);
            ...
            Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MODE, CurMode.mode - 0x98);    //Looks like the s3 bios
        ...
        switch (CurMode.cheight) {
            case 8:
                ...
                break;
            case 14:
                Memory.RealSetVec(0x43, Int10.int10.rom.font_14);
                break;
            case 16:
                Memory.RealSetVec(0x43, Int10.int10.rom.font_16);
                break;
        }
        ...
        switch (Dosbox.machine) {
            ...
            case MachineType.MCH_PCJR: //TANDY_ARCH_CASE:
                ...
                if (!SetCurMode(ModeList_OTHER, mode)) {
                    Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "Trying to set illegal mode " + Integer.toString(mode, 16));
                    return false;
                }
                break;
...
//	VGA_SetMode(CurMode.type);
        /* Setup the CRTC */
        /*Bitu*/
        int crtc_base = Dosbox.machine == MachineType.MCH_HERC ? 0x3b4 : 0x3d4;
        //Horizontal total
        ...
        /*Bit8u*/
        short mode_control, color_select;
        switch (Dosbox.machine) {
            case MachineType.MCH_HERC:
            ...
            case MachineType.MCH_TANDY:
                /* Init some registers */
                IO.IO_WriteB(0x3da, 0x1);
                IO.IO_WriteB(0x3de, 0xf);        //Palette mask always 0xf
                IO.IO_WriteB(0x3da, 0x2);
                IO.IO_WriteB(0x3de, 0x0);        //black border
                IO.IO_WriteB(0x3da, 0x3);                            //Tandy color overrides?
                switch (CurMode.mode) {
                    case 0x8:
                    case 0x9:
                        IO.IO_WriteB(0x3de, 0x14);
                        break;
                    case 0xa:
                        IO.IO_WriteB(0x3de, 0x0c);
                        break;
                    default:
            ...
            case MachineType.MCH_PCJR:
                ...
                IO.IO_ReadB(0x3da);
                IO.IO_WriteB(0x3da, 0x1);
                IO.IO_WriteB(0x3da, 0xf);        //Palette mask always 0xf
                IO.IO_WriteB(0x3da, 0x2);
                IO.IO_WriteB(0x3da, 0x0);        //black border
                ...
                IO.IO_WriteB(0x3da, 0x0);
                IO.IO_WriteB(0x3da, mode_control);
                Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MSR, mode_control);
                if (CurMode.mode == 0x6 || CurMode.mode == 0xa) color_select = 0x3f;
                else color_select = 0x30;
                Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_PAL, color_select);
    ...
    public static boolean INT10_SetVideoMode(/*Bit16u*/int mode) {
        ...
        /*Bit8u*/
        int modeset_ctl = Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_MODESET_CTL);
        if (Dosbox.IS_VGA_ARCH()) {
            ...
            switch (Dosbox.svgaCard) {
                ...
                case SVGACards.SVGA_TsengET3K:
                    if (!SetCurMode(ModeList_VGA_Tseng, mode)) {
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "VGA:Trying to set illegal mode " + Integer.toString(mode, 16));
                        return false;
                    }
                    break;
            ...
            if (!SetCurMode(ModeList_EGA, mode)) {
                Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "EGA:Trying to set illegal mode " + Integer.toString(mode, 16));
                return false;
            }
        }
        ...
        /*Bit8u*/
        int misc_output = 0x2 | (mono_mode ? 0x0 : 0x1);
        ...
        /* Program Sequencer */
        /*Bit8u*/
        short[] seq_data = new short[SEQ_REGS];
        ...
        for (/*Bit8u*/byte ct = 0x0; ct <= 0x18; ct++) {
            IoHandler.IO_Write(crtc_base, ct);
            IoHandler.IO_Write(crtc_base + 1, 0);
        }
        /*Bit8u*/
        short overflow = 0;/*Bit8u*/
        short max_scanline = 0;
        /*Bit8u*/
        short ver_overflow = 0;/*Bit8u*/
        short hor_overflow = 0;
        /* Horizontal Total */
        IoHandler.IO_Write(crtc_base, 0x00);
        IoHandler.IO_Write(crtc_base + 1, (CurMode.htotal - 5));
        hor_overflow |= ((CurMode.htotal - 5) & 0x100) >> 8;
        /* Horizontal Display End */
        IoHandler.IO_Write(crtc_base, 0x01);
        IoHandler.IO_Write(crtc_base + 1, (CurMode.hdispend - 1));
        hor_overflow |= ((CurMode.hdispend - 1) & 0x100) >> 7;
        /* Start horizontal Blanking */
        IoHandler.IO_Write(crtc_base, 0x02);
        IoHandler.IO_Write(crtc_base + 1, CurMode.hdispend);
        hor_overflow |= ((CurMode.hdispend) & 0x100) >> 6;
        /* End horizontal Blanking */
        /*Bitu*/
        int blank_end = (CurMode.htotal - 2) & 0x7f;
        IoHandler.IO_Write(crtc_base, 0x03);
        IoHandler.IO_Write(crtc_base + 1, (0x80 | (blank_end & 0x1f)));

        /* Start Horizontal Retrace */
        /*Bitu*/
        int ret_start;
        if ((CurMode.special & _EGA_HALF_CLOCK) != 0 && (CurMode.type != VGA.M_CGA2))
        ...
        IoHandler.IO_Write(crtc_base, 0x04);
        ...
        IoHandler.IO_Write(crtc_base, 0x05);
        IoHandler.IO_Write(crtc_base + 1, (ret_end | (blank_end & 0x20) << 2));

        /* Vertical Total */
        IoHandler.IO_Write(crtc_base, 0x06);
        IoHandler.IO_Write(crtc_base + 1, (CurMode.vtotal - 2));
        ...
        IoHandler.IO_Write(crtc_base, 0x10);
        ...
        IoHandler.IO_Write(crtc_base, 0x12);
        IoHandler.IO_Write(crtc_base + 1, CurMode.vdispend - 1);
        ...
        IoHandler.IO_Write(crtc_base, 0x15);
        IoHandler.IO_Write(crtc_base + 1, (CurMode.vdispend + vblank_trim));
        ...
        /* Vertical Blank End */
        IoHandler.IO_Write(crtc_base, 0x16);
        IoHandler.IO_Write(crtc_base + 1, (CurMode.vtotal - vblank_trim - 2));

        /* Line Compare */
        /*Bitu*/
        int line_compare = (CurMode.vtotal < 1024) ? 1023 : 2047;
        IoHandler.IO_Write(crtc_base, 0x18);
        ...
        IoHandler.IO_Write(crtc_base, 0x17);
        ...
        /*Bit8u*/
        short[] gfx_data = new short[GFX_REGS];
        ...
        /*Bit8u*/
        short[] att_data = new short[ATT_REGS];
        ...
        if ((modeset_ctl & 8) == 0) {
            ...
            IoHandler.IO_Write(0x3c0, 0x00); //Disable palette access
...
//dac_text16:
        ...
        /*Bit8u*/
        //int feature =
                Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_INITIAL_MODE);
        switch (CurMode.type) {
            case VGA.M_CGA2:
            ...
            case VGA.M_TEXT:
                switch (CurMode.mode) {
                    case 0:
                        Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MSR, 0x2c);
                        break;
                    case 1:
                        Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MSR, 0x28);
                        break;
                    case 2:
                        Memory.real_writeb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MSR, 0x2d);
                        break;
                    case 3:
                    case 7:
...
//	Memory.real_writeb(Int10.BIOSMEM_SEG,Int10.BIOSMEM_INITIAL_MODE,feature);
        if (Dosbox.svgaCard == SVGACards.SVGA_S3Trio) {
            ...
            IoHandler.IO_Write(crtc_base, 0x3a);
            IoHandler.IO_Write(crtc_base + 1, reg_3a);
            IoHandler.IO_Write(crtc_base, 0x31);
            IoHandler.IO_Write(crtc_base + 1, reg_31);    //Enable banked memory and 256k+ access
            IoHandler.IO_Write(crtc_base, 0x58);
            IoHandler.IO_Write(crtc_base + 1, 0x3);        //Enable 8 mb of linear addressing
            IoHandler.IO_Write(crtc_base, 0x38);
            IoHandler.IO_Write(crtc_base + 1, 0x48);    //Register lock 1
            IoHandler.IO_Write(crtc_base, 0x39);
            IoHandler.IO_Write(crtc_base + 1, 0xa5);    //Register lock 2
