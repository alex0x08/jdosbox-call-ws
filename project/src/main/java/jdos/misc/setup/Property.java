...
public abstract class Property {
	...
	public boolean CheckValue(Value in, boolean warn) {
        ...
        for (int i=0;i<suggested_values.size();i++) {
            Value v = suggested_values.elementAt(i);
            if (in.equals(v))
                return true;
        }
        if (warn) Log.log_msg("\""+ in +"\" is not a valid value for variable: "+propname+".\nIt might now be reset to the default value: "+ default_value);
        return false;
    }
    ...
    protected final Vector<Value> suggested_values = new Vector<>();
