...
public class BatchFile {
    ...
    String ReadLine() {
        ...
        if (!Dos_files.DOS_OpenFile(filename,128,file_handle)) {
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,"ReadLine Can't open BatchFile "+filename);
            close();
            return null;
        }
    ...
    boolean Goto(String where) {
        ...
        if (!Dos_files.DOS_OpenFile(filename,128,file_handle)) {
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,"SHELL:Goto Can't open BatchFile "+filename);
            close();
            return false;
        }
